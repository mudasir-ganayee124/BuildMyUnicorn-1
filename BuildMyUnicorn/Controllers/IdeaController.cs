...
namespace BuildMyUnicorn.Controllers
{
    public class IdeaController : WebController
    {
        public ActionResult Index(string id)
        {
            ...
            if (Model != null && Model.IdeaID.ToString() != id) State = (int)EntityState.Old;
            ViewBag.Video = new Master().GetSectionModuleVideo((int)Module.MyIdea, (int)ModuleSection.MyIdea_Ideaoutofhead);
            if (ResponseType.Redirect.ToString() == CheckModuleCourse(State, (int)ModuleSection.MyIdea_Ideaoutofhead))
            {
                return RedirectToAction("Index", "ModuleCourse", new
                {
                    ControllerName = "Idea",
                    ActionName = "Index",
                    ModuleName = Module.MyIdea.ToString(),
                    SectionName = ModuleSection.MyIdea_Ideaoutofhead.ToString()
                });
            }
            return View(State);
        }
        ...
        public string Add(Idea model)
        {
            if (model.EntityState == EntityState.New)
            {
                model.IdeaID = Guid.NewGuid();
            }
            return new IdeaManager().AddIdea(model);
        }
