    ...
    public class IdeaController : WebController
    {
        public ActionResult Index(string id)
        {
            ...
            if (string.IsNullOrEmpty(id))
                Count = new IdeaManager().ExistIdea(Guid.Empty);
            else
            {
                Guid IdeaID;
                bool isValid = Guid.TryParse(id, out IdeaID);
                if (isValid) Count = new IdeaManager().ExistIdea(IdeaID);
            ...
            if (State == (int)EntityState.New && string.IsNullOrEmpty(id))
            {
                ...
                objlog.ModuleID = Module.MyIdea;
                objlog.ModuleSectionID = ModuleSection.MyIdea_Ideaoutofhead;
                if (new Master().AddModuleCourselog(objlog).Status == (int)ResponseType.Redirect && new Master().ExistModuleCourse((int)Module.MyIdea, (int)ModuleSection.MyIdea_Ideaoutofhead) > 0)
                {
                    ...
                    return RedirectToAction("Index", "ModuleCourse", new
                    {
                        ControllerName = "Idea",
                        ActionName = "Index",
                        ModuleID = (int)Module.MyIdea,
                        SectionID = (int)ModuleSection.MyIdea_Ideaoutofhead
                    });
                }
            }

            ViewBag.Video = new Master().GetSectionModuleVideo((int)Module.MyIdea, (int)ModuleSection.MyIdea_Ideaoutofhead);
            return View(State);
        }
        ...
        {
             Idea obj = new Idea();
            _Idea Model = new IdeaManager().GetIdea();
            var GetModelDependency = Task.Factory.StartNew(() =>
            {
                GetIdeaDataDependency();
            ...
            return PartialView("_NewIdeaPartial", obj);
        }
        ...
        public ActionResult Detail()
        {
            ...
            ViewBag.RateInfo = new Master().GetRateInfo((int)Module.MyIdea, (int)ModuleSection.MyIdea_Ideaoutofhead);
            Task.WaitAll(new Task[] { GetModelData, GetModelDependency });

            return PartialView("_DetailIdeaPartial", obj);
        }

        public JsonResult AddIdea(Idea model)
        {
            if (model.EntityState == EntityState.New)
            {
                model.IdeaID = Guid.NewGuid();
            }
           
            return  Json(new IdeaManager().AddIdea(model), JsonRequestBehavior.AllowGet);
        }

        public void GetIdeaDataDependency()
        {

            ViewBag.ProductDemandList = new Master().GetOptionMasterList((int)OptionType.Idea_ProductDemand);
            ViewBag.InMarketAlreadyList = new Master().GetOptionMasterList((int)OptionType.Idea_InMarketAlready);
            ViewBag.ScalableList = new Master().GetOptionMasterList((int)OptionType.Idea_IsItScalable);
            ViewBag.EndGoalList = new Master().GetOptionMasterList((int)OptionType.Idea_EndGoal);
            ViewBag.CompanySetupList = new Master().GetOptionMasterList((int)OptionType.GeneralTwoOption);
            ViewBag.CofounderList = new Master().GetOptionMasterList((int)OptionType.GeneralTwoOption);
            ViewBag.SupportTechnicallyList = new Master().GetOptionMasterList((int)OptionType.Idea_SupportTechnically);
            ViewBag.BuildFromList = new Master().GetOptionMasterList((int)OptionType.Idea_BuildIt);
            ViewBag.SellToList = new Master().GetOptionMasterList((int)OptionType.Idea_SellingTo);
            ViewBag.SaleStaffPlanList = new Master().GetOptionMasterList((int)OptionType.Idea_PlanSalesStaff);
            ViewBag.BusinessCostLList = new Master().GetOptionMasterList((int)OptionType.Idea_CostLaunchYourBusiness);
            ViewBag.Language = new Master().GetDefaultModuleLanguage((int)Module.MyIdea, (int)ModuleSection.MyIdea_Ideaoutofhead);
            ViewBag.MasterStartup = new Master().GetOptionMasterList((int)OptionType.Startup);
            ViewBag.MasterTechnology = new Master().GetOptionMasterList((int)OptionType.Technology);
            ...
            ViewBag.ModuleQustionVideo = new Master().GetModuleQuestionVideo((int)Module.MyIdea, (int)ModuleSection.MyIdea_Ideaoutofhead);
        }
