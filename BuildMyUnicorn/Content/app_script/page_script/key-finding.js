$(document).ready(function () {
    ...
    $('.jsYear').each(function (index, value) {
        $(this).val(Number.parseFloat($(this).val()).toFixed(2).replace(/[^0-9,.]/gi, '') + ' years');
    });
    ...
    $(document).on("click", ".JsUpdateKeyfinding", function () {
        ...
        window.location.replace(GetBaseURL() + "KeyFinding/Index/" + KeyFindingID);
    });
    $(document).on("blur", "#MarketSize, #CaptureShare", function () {
        var inputValue = $("#MarketSize").val();
        if (inputValue)
        {
            if (inputValue.charAt(0) === "$") {
                inputValue = inputValue.substring(1);  }
        ...
        var Percentage = parseFloat(inputValue) * parseFloat(CaptureShare) / 100; 
        $("#MarketShare").val((Number.parseFloat(Percentage).toFixed(2).replace(/[^0-9,.]/gi, '') + '%'));
