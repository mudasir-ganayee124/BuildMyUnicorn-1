...
$("#frm_ProductService").steps({
    ...
    onCanceled: function (event) { 
       if ($("#IsRedirect").val() === "true") { window.location.replace(GetBaseURL() + "ModuleCourse?ControllerName=Business&ActionName=ProductService&ModuleID=3&SectionID=4"); }
    ...
    onFinished: function (event, currentIndex) {
        ...
        $.each($(".jsTechnologyRoadMap"), function () {
           ...
           var PSIPAddress =   $("#PSIPAddress").val();
           var PSTradeMark =   $("#PSTradeMark").val();
          if ($("#PSHaveIPAddress").val() !== "7f5d70c1-5e5b-4411-a05c-224976e6feff") 
              PSIPAddress = ""; 
          if ($("#PSHaveTradeMark").val() !== "7f5d70c1-5e5b-4411-a05c-224976e6feff") 
              PSTradeMark = ""; 

        //if ($("#PSHaveTechnologyRoadMap").val() === "7f5d70c1-5e5b-4411-a05c-224976e6feff") {
        //    $(".jsPSIPAddress").removeClass("hide");
        //}
        //if ($("#MVPHavePrototype").val() === "7f5d70c1-5e5b-4411-a05c-224976e6feff") {
        //    $(".jsPSIPAddress").removeClass("hide");
        //}

        var Model = {
            "ProductServiceID": $.trim($("#ProductServiceID").val()), "ClientID": $.trim($("#ClientID").val()), "EntityState": $.trim($("#EntityState").val()),
            "AboutProduct": { "PSSelling": PSSelling.join(","), "PSDescription": $.trim($("#PSDescription").val()), "PSProduce": PSProduce.join(","), "PSDevelopStart": $.trim($("#PSDevelopStart").val()), "PSReadylaunch": $.trim($("#PSReadylaunch").val()), "PSVariations": $.trim($("#PSVariations").val()), "PSHaveIPAddress": $.trim($("#PSHaveIPAddress").val()), "PSIPAddress": PSIPAddress, "PSHaveTradeMark": $.trim($("#PSHaveTradeMark").val()), "PSTradeMark": PSTradeMark, "PSHaveTechnologyRoadMap": $.trim($("#PSHaveTechnologyRoadMap").val()) },
