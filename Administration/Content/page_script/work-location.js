...
$(document).ready(() => {
    ...
    $("#Grid").kendoGrid({
        ...
        selectable: "row",
        messages: {
            noRecords: "No Work location Found."
        },
        ...
        columns: [{
            template: '<button class="btn btn-info" title="Edit Work location" onclick=Edit("#: ID #")><i class="icon-pencil"></i></button>\
                      <button class= "btn btn-danger" title="Delete Work location" onclick=Delete("#: ID #") > <i class="fa fa-trash"></i></button> ',
            width: 100
        },
            { field: "Value", title: "Work location", width: 140, filterable: false },
...
$("#UpdateForm").submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: GetBaseURL() + "WorkLocation/Update",
        method: "POST",
        ...
        success: function (response) {
            if (response === "OK") {
                ...
                swal("Success!", "Work location Updated Successfully", "success");
...
$("#NewForm").submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: GetBaseURL() + "WorkLocation/Add",
        method: "POST",
        ...
        success: function (response) {
            if (response === "OK") {
                ...
                swal("Success!", "Work location Added Successfully", "success");
                $('#Grid').data('kendoGrid').dataSource.read();
...
function Edit(Value) {
    ...
    $.ajax({
        url: GetBaseURL() + "WorkLocation/Get/",
        method: "GET",
        data: { ID: Value },
        dataType: 'json',
            ...
            deleteUser: {
                ...
                action: function () {
                    ...
                    $.fn.ajaxCall(option).done(function (response) {
                        ...
                        if (response === "OK") {
                            $.fn.successMsg("Work location deleted successfully");
                            $('#Grid').data('kendoGrid').dataSource.read();
                        }
                        else
