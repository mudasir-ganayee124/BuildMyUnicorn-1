...
$(document).ready(() => {
    ...
    $("#Grid").kendoGrid({
        ...
        selectable: "row",
        messages: {
            noRecords: "No  Business Placement Found."
        },
        ...
        columns: [{
            template: '<button class="btn btn-info" title="Edit Business Placement" onclick=Edit("#: ID #")><i class="icon-pencil"></i></button>\
                      <button class= "btn btn-danger" title="Delete Business Placement" onclick=Delete("#: ID #") > <i class="fa fa-trash"></i></button> ',
            width: 100
        },
...
$("#UpdateForm").submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: GetBaseURL() + "BusinessPlacement/Update",
        method: "POST",
        ...
        success: function (response) {
            if (response === "OK") {
                ...
                swal("Success!", "Business Placement Updated Successfully", "success");
...
$("#NewForm").submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: GetBaseURL() + "BusinessPlacement/Add",
        method: "POST",
        ...
        success: function (response) {
            if (response === "OK") {
                ...
                swal("Success!", "Business Placement Added Successfully", "success");
                $('#Grid').data('kendoGrid').dataSource.read();
...
function Edit(Value) {
    ...
    $.ajax({
        url: GetBaseURL() + "BusinessPlacement/Get/",
        method: "GET",
        data: { ID: Value },
        dataType: 'json',
            ...
            deleteUser: {
                ...
                action: function () {
                    ...
                    $.fn.ajaxCall(option).done(function (response) {
                        ...
                        if (response === "OK") {
                            $.fn.successMsg("Business Placement deleted successfully");
                            $('#Grid').data('kendoGrid').dataSource.read();
                        }
                        else
