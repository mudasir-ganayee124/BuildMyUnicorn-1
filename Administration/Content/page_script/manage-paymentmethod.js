...
$(document).ready(() => {
    ...
    $("#liManagePaymentMethod").addClass("active");
    ...
    $("#Grid").kendoGrid({
        dataSource: {
            ...
            transport: {
                read: {
                    url: "/ManagePaymentMethod/GetAll"
        ...
        columns: [{
            template: '<button class="btn btn-info" title="Edit Payment Method" onclick=Edit("#: ID #")><i class="icon-pencil"></i></button>\
                      <button class= "btn btn-danger" title="Delete Payment Method" onclick=Delete("#: ID #") > <i class="fa fa-trash"></i></button> ',
            width: 100
        },
            { field: "Value", title: "Third Party", width: 140, filterable: false },
...
$("#UpdateForm").submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: GetBaseURL() + "ManagePaymentMethod/Update",
        method: "POST",
...
$("#NewForm").submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: GetBaseURL() + "ManagePaymentMethod/Add",
        method: "POST",
...
function Edit(Value) {
    ...
    $.ajax({
        url: GetBaseURL() + "ManagePaymentMethod/Get/",
        method: "GET",
        data: { ID: Value },
        dataType: 'json',
        ...
        buttons: {
            deleteUser: {
                ...
                action: function () {
                    var option = {
                        action: "Delete",
                        controller: "ManagePaymentMethod",
                        dataType: "text",
                        data: { ID: Value }
                    };
