...
$(document).ready(() => {
    ...
    $("#Grid").kendoGrid({
        ...
        columns: [{
            template: '<button class="btn btn-info" title="Edit Money Raise" onclick=Edit("#: ID #")><i class="icon-pencil"></i></button>\
                      <button class= "btn btn-danger" title="Delete Money Raise" onclick=Delete("#: ID #") > <i class="fa fa-trash"></i></button> ',
            width: 100
        },
...
$("#UpdateForm").submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: GetBaseURL() + "MoneyRaise/Update",
        method: "POST",
            ...
            deleteUser: {
                ...
                action: function () {
                    ...
                    $.fn.ajaxCall(option).done(function (response) {
                        ...
                        if (response === "OK") {
                            $.fn.successMsg("Money aise deleted successfully");
                            $('#Grid').data('kendoGrid').dataSource.read();
                        }
                        else
