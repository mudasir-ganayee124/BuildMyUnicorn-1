...
$("#ImageUpload").on('change', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data !== "!OK") {
                $("#ImageID").val(data);
                $("#profileImageDialog").attr('src', GetBaseURL() + "Content/Images/" + data + "");
...
$("#frm_UpdateProfile").submit(function(e) {
       ...
       $.each($("input[name='_CompanyType']:checked"), function () {
        CompanyType.push($(this).val());
       });

     $("#BusinessPlacement").val(BusinessPlacement.join(","));
    $("#WorkLocation").val(Worklocation.join(","));
    $("#CompanyType").val(CompanyType.join(","));
          $.ajax({
